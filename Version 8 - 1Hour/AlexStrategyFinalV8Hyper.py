...
from freqtrade.strategy import IStrategy, RealParameter, DecimalParameter

logger = logging.getLogger(__name__)

class AlexStrategyFinalV8Hyper(IStrategy):
    """
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: Dict) -> DataFrame:
        ...
        dataframe['V_mean'] = 1 / bb_width
        dataframe['V2_mean'] = 1 / dataframe['atr']
        
        mean_v = dataframe['V_mean'].mean()
        std_v = dataframe['V_mean'].std()
        dataframe['V_norm'] = (dataframe['V_mean'] - mean_v) / std_v

        mean_v2 = dataframe['V2_mean'].mean()
        std_v2 = dataframe['V2_mean'].std()
        dataframe['V2_norm'] = (dataframe['V2_mean'] - mean_v2) / std_v2

        dataframe['V'] = np.where(dataframe['V_norm'] > 1, 1, np.where(dataframe['V_norm'] < -1, -1, 0))
        dataframe['V2'] = np.where(dataframe['V2_norm'] > 1, 1, np.where(dataframe['V2_norm'] < -1, -1, 0))
        ...
        target_horizon = 2  # Define your prediction horizon here
